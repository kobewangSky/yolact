...
class make_net_iou(nn.Module):
    """
    A helper function to take a config setting and turn it into a network.
    Used by protonet and extrahead. Returns (network, out_channels)
    """
    def __init__(self, in_channels, conf, include_last_relu=True):
        super(make_net_iou, self).__init__()

        self.in_channels = in_channels
        # Use sum to concat together all the component layer lists
        net = sum([self.make_layer(x) for x in conf], [])
        if not include_last_relu:
            net = net[:-1]

        self.add_module("make_net_iou", nn.Sequential(*(net)))



    def make_layer(self, layer_cfg):
        #nonlocal in_channels
        ...
        if isinstance(layer_cfg[0], str):
            ...
            if layer_name == 'cat':
                nets = [make_net(self.in_channels, x) for x in layer_cfg[1]]
                layer = Concat([net[0] for net in nets], layer_cfg[2])
                num_channels = sum([net[1] for net in nets])
        else:
            num_channels = layer_cfg[0]
            ...
            if kernel_size > 0:
                layer = nn.Conv2d(self.in_channels, num_channels, kernel_size, **layer_cfg[2])
            else:
                if num_channels is None:
                    layer = InterpolateModule(scale_factor=-kernel_size, mode='bilinear', align_corners=False,
                                              **layer_cfg[2])
                else:
                    layer = nn.ConvTranspose2d(self.in_channels, num_channels, -kernel_size, **layer_cfg[2])

        self.in_channels = num_channels if num_channels is not None else self.in_channels
